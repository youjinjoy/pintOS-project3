...
/* Search current thread's child_list and return child with pid. Return NULL if
 * not found. */
...
tid_t process_fork(const char *name, struct intr_frame *if_) {
  ...
  struct thread *cur = thread_current();
  memcpy(&cur->parent_if, if_,
         sizeof(struct intr_frame)); // Pass this intr_frame down to child
...
static bool duplicate_pte(uint64_t *pte, void *va, void *aux) {
...
#endif
    return true; // return false ends pml4_for_each, which is undesirable - just
                 // return true to pass this kernel va
  } else {
#ifdef DEBUG
  ...
  printf("Is user %d, is kernel %d, writable %d\n", is_user_pte(pte),
         is_kern_pte(pte), is_writable(pte));
#endif
  ...
  if (parent_page == NULL) {
    printf(
        "[fork-duplicate] failed to fetch page for user vaddr 'va'\n"); // #ifdef
                                                                        // DEBUG
    return false;
  }
  ...
  void *test = ptov(PTE_ADDR(*pte)) +
               pg_ofs(va);         // should be same as parent_page -> Yes!
  uint64_t va_offset = pg_ofs(va); // should be 0; va comes from PTE, so there
                                   // must be no 12bit physical offset
#endif
...
int process_exec(void *f_name) {
  ...
  char *argv[30]; // Q. 테스트는 일단 통과, 사이즈 30이면 충분하겠지? 동적할당
                  // 안해도 되겠지?
  int argc = 0;
  ...
  // hex_dump(_if.rsp, _if.rsp, USER_STACK - (uint64_t)*rspp, true); // #ifdef
  // DEBUG
...
void process_exit(void) {
  ...
  */
  ...
  // Postpone child termination until parents receives its exit status with
  // 'wait'
